...
// public function create(string $name, string $description, float $price, int $category_id, DateTime $created

   public function create(string $name, string $description, int $price, int $category_id, DateTime $created): bool {
    $stmt = $this->conn->prepare("
      INSERT INTO {$this->itemsTable} (`name`, `description`, `price`, `category_id`, `created`)
      VALUES (?, ?, ?, ?, ?)
    ");

    $stmt->bind_param('ssiis', $name, $description, $price, $category_id, $created->format('YYYY-MM-DD HH:MM:SS'));
